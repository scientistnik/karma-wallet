  ...
  <div>
    ...
    <div class="card">
      ...
      <div class="card-context">
        ...
        <b-field><button class="button is-primary" :class="{'is-loading':isLoading}" @click="transfer()">Transfer</button></b-field>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async transfer() {
      ...
      this.isLoading = true
      try {
        await account.transfer(to.name, 'KRMT', +this.amount)
        console.log("transfer finished!")
        this.$snackbar.open('Transfered')
      } catch(err) {
        this.$snackbar.open(`Error when transfer: ${err}`)
      }
